...
export function UserNav() {
    ...
    const initials = (user.name?.substring(0, 2) || user.email?.substring(0, 2) || 'P').toUpperCase();
            ...
            <DropdownMenuContent className="w-56 bg-card/95 backdrop-blur-xl border-border/50 rounded-2xl p-2" align="end" forceMount>
                <DropdownMenuLabel className="font-normal p-2">
                    <div className="flex flex-col space-y-1">
                        <p className="text-sm font-bold leading-none text-foreground">{user.name || 'Focused Human'}</p>
